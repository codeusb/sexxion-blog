...
&emsp;&emsp;缓存未命中后，当用户按下回车键，Browser 进程的 UI 线程会通过进程间通信（IPC）通知 Network Service 初始化一个网络请求，进而**开始导航**，这时候 tab 标签页上会展示一个提示资源正在加载中的旋转圈圈。现代 Chromium 中，网络请求通常由独立的 Network Service 进程负责处理，Network Service 内部再通过相关线程或线程池完成 DNS、TCP、TLS、HTTP 等具体网络工作。随后 Network Service 进行 DNS 寻址，将域名转换为服务器 IP 地址。
...
1. `UI线程会通过 IPC 通知 Network Service` 进程间通信是如何完成的？
2. `Network Service` 网络服务、网络进程与内部网络线程之间的关系？
...
&emsp;&emsp;服务器将响应发回客户端，Network Service 接收响应头后，**在响应头阶段**即根据 `Content-Type` 等信息开始判断 MIME 类型，而不是等全部数据接收完毕。

**重定向处理**

如果服务器返回的是 **HTTP 301/302 重定向响应**，Network Service 会通过 IPC 告知 Browser 进程进行地址跳转，由 Browser 进程的 UI 线程更新地址栏，再通知 **Network Service** 重新发起一个新的网络请求（通常会重新走导航相关流程，是否重新 DNS 寻址取决于 DNS 缓存、连接复用、协议版本等因素）。
...
&emsp;&emsp;浏览器的 **Network Service** 在接收到响应头时即开始处理（流式处理，不等响应体全部到达）。根据 HTTP 头部的 `Content-Type` 判断 MIME 类型：

- 是 HTML 文件 → 通过 Browser 进程协调导航，并通知**渲染进程**准备渲染
- 是压缩文件（如 zip）→ 交给下载管理器处理
- 是其他资源（图片、JS、CSS 等）→ 交给对应的资源加载与解析流程处理

Network Service 处理响应数据后，会通过 IPC 与 Browser 进程、Renderer 进程协作，将导航结果和资源数据交给渲染进程继续处理。

### 3.2 浏览器渲染

&emsp;&emsp;_ps：进入前端领域！_

注：**现代浏览器中，JavaScript 执行、HTML 解析、样式计算、布局等主要工作通常发生在渲染进程的主线程（Main Thread）上。JS 长时间执行会占用主线程，从而阻塞 HTML 解析、样式计算、布局和绘制等渲染相关工作。**
...
5. **布局（Layout/Reflow重排）**：在渲染主线程中计算布局树中各元素的尺寸和位置。

6. **绘制（Paint/Repaint重绘）**：在渲染主线程中为每个图层生成绘制指令，描述如何绘制该层的像素信息。

7. **光栅化（Rasterize）**：合成线程将图层切分为图块（Tiles），由光栅化线程将图块转化为位图。

8. **合成与显示（Composite）**：合成线程提交合成帧，浏览器将相关信息交给 GPU 进程，GPU 完成合成并最终显示在屏幕上。
...
&emsp;&emsp;当 HTML 解析器遇到普通 `<script>` 标签时，会暂停 HTML 解析，执行脚本，执行完毕后再继续解析。由于 JavaScript 执行、HTML 解析、样式计算和布局等都依赖渲染进程的主线程，因此长时间执行 JavaScript 会阻塞页面渲染。

> 补充：浏览器有**预加载扫描器（Preload Scanner）**，它在主线程被 JS 阻塞时，仍可在后台继续扫描 HTML，提前发起对后续图片、CSS、脚本等资源的加载请求，从而减少整体渲染耗时。

_思考_

1. `JS 执行会阻塞页面渲染` 为什么这样设计？
